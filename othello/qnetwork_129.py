...
class QNetwork129(object):
    ...
    def update(self,Q,sBoard,action,r,sPrimeBoard,sess):
        ...
        @param tfSession sess
        """
        Q1 = sess.run(self.Qout,feed_dict={self.inputLayer:np.identity(129)[sPrimeBoard]})
        maxQ1 = np.max(Q1)
        targetQ = Q
        targetQ[0,action] = r + self.y*maxQ1
        _ = sess.run(self.updateModel,feed_dict={self.inputLayer:np.identity(129)[sBoard],self.nextQ:targetQ})
