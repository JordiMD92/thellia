...
class QNetworkTangent(QNetwork):
    def __init__(self,lr):
        ...
        hidden = tf.layers.dense(self.inputLayer, 200, activation=tf.tanh)
        hidden = tf.layers.dense(hidden, 150, activation=tf.tanh)
        hidden = tf.layers.dense(hidden, 100, activation=tf.tanh)
