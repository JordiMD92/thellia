...
def gameMode(num_episodes,train,sess):
    ...
    if gameMode == GameMode['hvh']:
        ...
        #DQN vs DQN
        return QPlayer(1,QN,targetQN,train,num_episodes,sess,trainables), QPlayer(-1,QN,targetQN,train,num_episodes,sess,trainables)
    elif gameMode == GameMode['qvh']:
        #DQN vs Human
        return QPlayer(1,QN,targetQN,train,num_episodes,sess,trainables), HumanPlayer(-1,view)
    elif gameMode == GameMode['qvr']:
        #DQN vs Random
        return QPlayer(1,QN,targetQN,train,num_episodes,sess,trainables), RandomPlayer(-1)
...
def loadDB(modelPath,model,sess):
    ...
    if num_games:
        p1 = QPlayer(1,QN,targetQN,"load",num_games,sess,trainables)
        p2 = QPlayer(-1,QN,targetQN,"load",num_games,sess,trainables)
